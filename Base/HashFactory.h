...
namespace HashFactory
{
	...
	namespace Crypto
	{
		...
		IHash CreateBlake2B(const Blake2BConfig& a_Config);

		IHash CreateBlake2B(const Blake2BConfig& a_Config, const Blake2BTreeConfig& a_TreeConfig);
		...
		IHash CreateBlake2S(const Blake2SConfig& a_Config);

		IHash CreateBlake2S(const Blake2SConfig& a_Config, const Blake2STreeConfig& a_TreeConfig);
	...
	namespace XOF
	{
		IXOF CreateBlake2XB(const Blake2XBConfig& config, const UInt64 xofSizeInBits);
		IXOF CreateBlake2XB(const HashLibByteArray& key, const UInt64 xofSizeInBits);

		IXOF CreateBlake2XS(const Blake2XSConfig& config, const UInt64 xofSizeInBits);
