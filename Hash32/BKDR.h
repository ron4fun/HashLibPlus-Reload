...
class BKDR : public Hash, public virtual IIHash32, public virtual IITransformBlock
{
public:
	BKDR()
		: Hash(4, 1)
	...
	{
		BKDR HashInstance = BKDR();
		HashInstance._hash = _hash;
		HashInstance.SetBufferSize(GetBufferSize());

		return IHash(new BKDR(HashInstance));
	}
	...
	virtual void TransformBytes(const HashLibByteArray& a_data, const Int32 a_index, const Int32 a_length)
	{
		...
		while (length > 0)
		{
			_hash = (_hash * UInt32(SEED)) + a_data[i];
			i++;
			length--;
		} // end while
...
private:
	...
	static const Int32 SEED = Int32(131);
